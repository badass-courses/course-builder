...
export default async function WorkshopAccessPage(props: Props) {
	...
	return (
		...
		>
				...
				<div className="grid w-full grid-cols-1 items-center gap-24 md:grid-cols-2 md:gap-10">
					<div className="flex flex-col gap-4">
						<h1 className="mb-4 text-3xl font-bold sm:text-4xl lg:text-5xl">
							Unlock the <strong>{workshopTitle}</strong> kit for free
						</h1>
						<div className="">
							...
							<ul className="flex flex-col space-y-2 md:text-lg">
								...
								<li className="flex items-center gap-2">
									...
									<span>All assets & code used in the video</span>
								</li>
								<li className="flex items-center gap-2">
									<span className="mt-1">
										<CheckmarkIcon />
									</span>
									<span>Progress tracking & certificate</span>
								</li>
							</ul>
						</div>
					</div>
					<div className="relative flex w-full flex-col items-center md:ml-auto md:max-w-lg">
						{workshop.fields.coverImage?.url && (
							<CldImage
								...
								width={219}
								height={123}
								className="absolute -top-16 rounded-lg"
							/>
						)}
						<Login
							className={cn(
								'border-primary w-full [&_button]:h-12 [&_h1]:text-2xl md:[&_h1]:text-3xl [&_input]:h-12',
								{
			...
			<div className="flex flex-col gap-4 px-5 pt-10 md:px-0 md:pt-32">
