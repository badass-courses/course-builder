...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    FROM
      vbd_ContentResource AS workshop
    LEFT JOIN vbd_ContentResourceResource AS section_relations
      ON workshop.id = section_relations.resourceOfId
    LEFT JOIN vbd_ContentResource AS sections
      ON sections.id = section_relations.resourceId AND sections.type = 'section'
    LEFT JOIN vbd_ContentResourceResource AS lesson_relations
      ON sections.id = lesson_relations.resourceOfId
    LEFT JOIN vbd_ContentResource AS lessons
      ON lessons.id = lesson_relations.resourceId AND (lessons.type = 'lesson' OR lessons.type = 'exercise')
    LEFT JOIN vbd_ContentResourceResource AS top_level_lesson_relations
      ON workshop.id = top_level_lesson_relations.resourceOfId
    LEFT JOIN vbd_ContentResource AS top_level_lessons
  ...
  `)
	...
	// Add other content types (articles, tips)
