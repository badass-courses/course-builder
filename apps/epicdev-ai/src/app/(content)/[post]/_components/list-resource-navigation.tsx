...
export default function ListResourceNavigation({
	...
	if (isListLoading) {
		return (
			<div
				className={cn(
					'bg-muted/50 scrollbar-thin top-(--nav-height) sticky flex h-[calc(100vh-var(--nav-height))] w-full max-w-[320px] flex-shrink-0 items-start justify-start overflow-y-auto border-r p-5',
					className,
					{ 'w-0': !isExpanded },
				)}
			>
	...
	return (
		<>
			<aside
				className={cn(
					'bg-muted/50 scrollbar-thin top-(--nav-height) sticky hidden h-[calc(100vh-var(--nav-height))] w-full max-w-[320px] flex-shrink-0 overflow-y-auto border-r xl:block',
					className,
					{
						'w-0': !isExpanded,
					},
...
export function MobileListResourceNavigation() {
	...
	return (
		<>
			<div className="bg-card fixed left-1 top-1.5 z-50 flex scale-90 items-center gap-4 rounded-lg border py-1 pl-1 pr-6 shadow-sm xl:sr-only xl:hidden">
				<Button
					className="rounded"
					onClick={() => {
						setIsOpen((prev) => !prev)
