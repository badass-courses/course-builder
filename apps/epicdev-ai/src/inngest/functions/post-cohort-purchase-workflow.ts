...
export const postCohortPurchaseWorkflow = inngest.createFunction(
	...
	async ({ event, step, db: adapter }) => {
		...
		// TODO: TAKEN FROM AI HERO - update this to use the correct dates, links, etc
		const liveOfficeHoursEmailProps = {
			eventTitle: 'Epic AI Live Office Hours',
		...
		if (isTeamPurchase) {
			...
			await step.run(`send welcome email to team purchaser`, async () => {
				...
				await sendAnEmail({
					...
					Subject: `Welcome to ${cohortResource.fields.title || 'Epic AI'}!`,
					To: user.email,
