...
export async function subscribeToNewsletter(
	...
	try {
		...
		if (!preferenceType || !preferenceChannel) {
			// Fallback: if communication system isn't seeded, still succeed
			// but log a warning
			console.warn(
		...
		if (existingPreference) {
			// User already subscribed - reactivate if previously opted out
 ...
 * Called from the confirmed page to persist subscription state.
 *
