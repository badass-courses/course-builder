 ...
 * - Timing utilities: withTiming wrapper for async functions
 *
 * @module logger
 */
 ...
 *
 * // With timing
 * const result = await withTiming('process-order', async () => {
 *   // ... do work ...
 ...
 *
 * Automatically logs the duration of the function execution and captures errors.
 * Attaches timing information to the current log context if available.
 *
 * @param name - Name of the operation being timed
 * @param fn - The async function to time
 * @param onError - Optional error handler (defaults to logging the error)
 * @returns The return value of the function
 *
 * @example
 * ```ts
 * const user = await withTiming('fetch-user', async () => {
 *   return db.user.findUnique({ where: { id: userId } })
 * })
 * // Logs: operation.timing { name: 'fetch-user', duration: 45, success: true }
 * ```
 *
 * @example
 * ```ts
...
export async function withTiming<T>(
	...
	try {
		...
		await log.debug('operation.timing', {
			name,
			duration,
			success: true,
		})
		...
		await log.error('operation.timing', {
			name,
			duration,
			success: false,
		...
		// Re-throw to preserve error flow
		throw error
	}
}
