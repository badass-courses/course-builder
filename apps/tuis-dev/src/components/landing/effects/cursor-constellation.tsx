...
const NODES = [
  { homeCol: 15, homeRow: 6, freqX: 1, freqY: 2, radiusX: 6, radiusY: 3, phaseX: 0, phaseY: 0 },
  { homeCol: 40, homeRow: 20, freqX: 2, freqY: 1, radiusX: 5, radiusY: 4, phaseX: 1, phaseY: 0.5 },
  { homeCol: 70, homeRow: 8, freqX: 1, freqY: 3, radiusX: 7, radiusY: 2, phaseX: 2, phaseY: 1 },
  { homeCol: 85, homeRow: 18, freqX: 3, freqY: 1, radiusX: 4, radiusY: 3, phaseX: 0.5, phaseY: 2 },
  { homeCol: 50, homeRow: 13, freqX: 2, freqY: 2, radiusX: 8, radiusY: 3, phaseX: 1.5, phaseY: 0.8 },
  { homeCol: 25, homeRow: 20, freqX: 1, freqY: 1, radiusX: 5, radiusY: 4, phaseX: 3, phaseY: 1.5 },
  { homeCol: 60, homeRow: 22, freqX: 2, freqY: 3, radiusX: 6, radiusY: 2, phaseX: 0.8, phaseY: 2.5 },
];

function getNodePos(node: typeof NODES[number], t: number): { col: number; row: number } {
  const col = Math.round(node.homeCol + Math.cos(t * node.freqX + node.phaseX) * node.radiusX);
  const row = Math.round(node.homeRow + Math.sin(t * node.freqY + node.phaseY) * node.radiusY);
  return {
    ...
    for (let i = 0; i < positions.length; i++) {
      for (let j = i + 1; j < positions.length; j++) {
        ...
        if (dist < CONNECTION_RANGE) {
          const lineIntensity = Math.floor((1 - dist / CONNECTION_RANGE) * 5) + 2;
          drawLine(grid, intensityGrid, a.col, a.row, b.col, b.row, lineIntensity);
        }
      }
    }
