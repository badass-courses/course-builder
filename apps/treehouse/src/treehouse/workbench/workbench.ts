...
export class Workbench {
  ...
  async initialize() {
    ...
    if (this.workspace.lastOpenedID) {
      this.openNewPanel(
        this.workspace.find(this.workspace.lastOpenedID) ||
          this.workspace.mainNode(),
      );
    } else {
    ...
    if (this.workspace.settings.theme) {
      ...
      css.setAttribute(
        "href",
        `https://treehouse.sh/style/themes/${this.workspace.settings.theme}.css`,
      );
  ...
  authenticated(): boolean {
    return (
      Boolean(this.backend.auth && this.backend.auth.currentUser()) || false
    );
  }

  openQuickAdd() {
    let node =
      this.workspace.find("@quickadd") || this.workspace.new("@quickadd");

    this.showDialog(
      () => React.createElement(QuickAdd, { workbench: this, node }),
      true,
    );
    setTimeout(() => {
      (
  ...
  todayNode(): Node {
    ...
    const dayNode = today
      .toUTCString()
      .split(today.getFullYear().toString())[0];
  ...
  showMenu(event: React.MouseEvent, ctx: Context, style?: {}) {
    ...
    const cmds =
      items
        .filter((i) => i.command)
        .map((i) => this.commands.commands[i.command] as Command) || [];
    if (!items) return;
    this.menu = Menu({
  ...
  showPalette(x: number, y: number, ctx: Context) {
    this.showDialog(
      () => React.createElement(CommandPalette, { workbench: this, ctx }),
      false,
      {
        left: `${x}px`,
        top: `${y}px`,
  ...
  showSettings() {
    this.showDialog(
      () => React.createElement(Settings, { workbench: this }),
      true,
    );
  }
