...
import { CLOUDINARY_WEBHOOK_EVENT, CloudinaryWebhookEventSchema } from '@/inngest/events/cloudinary-webhook'
import { inngest } from '@/inngest/inngest.server'
import { withSkill } from '@/server/with-skill'

export const POST = withSkill(async (req: NextRequest) => {
  // todo: check secret to verify the request
  const cloudinaryWebhookEvent = CloudinaryWebhookEventSchema.parse(await req.json())
