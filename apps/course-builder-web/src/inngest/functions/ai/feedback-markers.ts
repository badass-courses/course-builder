  ...
  async ({ event, step }) => {
    const markers = await step.run('Generate Feedback Markers', async () => {
      return fetch('https://api.openai.com/v1/chat/completions', {
        ...
        .then((response) => {
          return z.array(FeedbackMarkerSchema).parse(JSON.parse(response.choices[0].message.content).markers)
        })
        .catch((e) => {
          console.error(e)
          return []
    ...
    await step.run('Broadcast Completion', async () => {
      await fetch(`${env.NEXT_PUBLIC_PARTY_KIT_URL}/party/${event.data.resourceId}`, {
        method: 'POST',
        body: JSON.stringify({
          body: markers,
