...
export const postPurchaseWorkflow = inngest.createFunction(
	...
	async ({ event, step, db: adapter, paymentProvider }) => {
		...
		await step.run('grant coupon entitlements for new purchase', async () => {
			...
			const isRestricted = purchase.status === 'Restricted'
			const isValid = purchase.status === 'Valid'

			let amountInCents: number | null = null
			if (isValid) {
				const productPrice = await adapter.getPriceForProduct(
					purchase.productId,
				)
				if (productPrice?.unitAmount) {
					amountInCents = Math.round(Number(productPrice.unitAmount) * 100)
					...
					return {
						...
						productId: purchase.productId,
						productPrice,
						debug: 'Product price lookup returned null/undefined',
					}
				}
			} else if (isRestricted) {
				amountInCents = Math.round(Number(purchase.totalAmount) * 100)
			}

			const anyCouponsForProduct = await db.query.coupon.findMany({
			...
			// If no coupons exist for this product, skip (product not set up for coupon credits)
			...
			// Check if user already has an entitlement for this eligibility product
			const existingEntitlementForProduct =
				await db.query.entitlements.findFirst({
					where: (entitlements, { and, eq, isNull, sql }) =>
						and(
							...
							sql`JSON_EXTRACT(${entitlements.metadata}, '$.eligibilityProductId') = ${purchase.productId}`,
							isNull(entitlements.deletedAt),
						),
				})

			if (existingEntitlementForProduct) {
				await log.info('coupon.entitlements.already_exists_for_product', {
					userId: user.id,
					productId: purchase.productId,
					existingEntitlementId: existingEntitlementForProduct.id,
					existingEntitlementSourceId: existingEntitlementForProduct.sourceId,
					reason:
			...
			let matchingCoupon: (typeof anyCouponsForProduct)[number] | undefined =
				undefined

			if (isRestricted) {
				matchingCoupon = anyCouponsForProduct.find(
					(c) => c.amountDiscount === amountInCents,
				)

				let couponToGrant = matchingCoupon

				if (!couponToGrant && amountInCents && amountInCents > 0) {
					try {
						...
						couponToGrant = await db.query.coupon.findFirst({
						...
						await log.error(
							'coupon.entitlements.coupon_creation_failed_for_restricted',
							{
								amountInCents,
								productId: purchase.productId,
								userId: purchase.userId,
						...
						return {
							checked: true,
							error: 'Failed to create coupon for Restricted purchase',
						}
					}
				}
				...
				if (couponToGrant) {
					const existingEntitlement = await db.query.entitlements.findFirst({
						where: (entitlements, { and, eq, isNull }) =>
							and(
								...
								eq(entitlements.sourceId, couponToGrant.id),
								eq(entitlements.sourceType, EntitlementSourceType.COUPON),
								eq(
					...
					if (!existingEntitlement) {
						const entitlementId = `${couponToGrant.id}-${guid()}`
						...
						granted++
						grantedCouponIds.push(couponToGrant.id)
						...
						await log.info(
							'coupon.entitlements.granted_for_restricted_purchase',
							{
								...
								couponId: couponToGrant.id,
								amountInCents,
							},
						)
					}
				}
			} else if (isValid && amountInCents && amountInCents > 0) {
				...
				const couponAmounts = anyCouponsForProduct.map((c) => ({
					couponId: c.id,
					amountDiscount: c.amountDiscount,
					amountDiscountType: typeof c.amountDiscount,
					amountDiscountAsNumber: c.amountDiscount
						? Number(c.amountDiscount)
						: null,
				}))

				await log.info('coupon.entitlements.coupon_amounts_debug', {
					productUnitPriceInCents: amountInCents,
					couponAmounts,
				})

				matchingCoupon = anyCouponsForProduct.find((c) => {
				...
				if (matchingCoupon) {
					const coupon = matchingCoupon
					await log.info('coupon.entitlements.matching_coupon_found', {
						couponId: coupon.id,
						couponAmountDiscount: coupon.amountDiscount,
						productUnitPriceInCents: amountInCents,
					})
					...
					await log.warn('coupon.entitlements.no_matching_coupon_found', {
						productUnitPriceInCents: amountInCents,
						availableCouponAmounts: couponAmounts
							.map((c) => c.amountDiscountAsNumber)
							.filter(Boolean),
					})
			...
			const debugInfo: Record<string, any> = {
				checked: true,
				matchingCoupons: anyCouponsForProduct.length,
				granted,
			...
			if (amountInCents) {
				debugInfo.amountInCents = amountInCents
			}

			if (isValid) {
				debugInfo.productUnitPriceInCents = amountInCents
				debugInfo.purchaseAmount = purchase.totalAmount
				debugInfo.couponAmounts = anyCouponsForProduct.map((c) => ({
					couponId: c.id,
					amountDiscount: c.amountDiscount,
					amountDiscountType: typeof c.amountDiscount,
					amountDiscountAsNumber: c.amountDiscount
				...
				debugInfo.matchingCouponFound = !!matchingCoupon
				if (matchingCoupon) {
					debugInfo.matchingCouponId = matchingCoupon.id
					debugInfo.matchingCouponAmount = matchingCoupon.amountDiscount
				} else {
					debugInfo.availableCouponAmounts = anyCouponsForProduct
						.map((c) => (c.amountDiscount ? Number(c.amountDiscount) : null))
