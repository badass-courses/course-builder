...
const resourceTeaserVariants = cva('', {
	variants: {
		variant: {
			list: 'flex w-full lg:flex-row flex-col items-center gap-6 lg:gap-16',
...
const thumbnailVariants = cva(
	'relative overflow-hidden bg-muted text-muted-foreground',
	{
		variants: {
			variant: {
				list: 'aspect-video w-full flex items-center justify-center [&_*]:transition-transform [&>*]:ease-in-out [&>*]:duration-300  group-hover:[&>*]:scale-102 max-w-[568px] shrink-0 rounded-xl lg:w-[568px]',
				card: 'aspect-video [&_*]:transition-transform [&>*]:ease-in-out [&>*]:duration-300  group-hover:[&>*]:scale-102 w-full flex items-center justify-center transition-transform duration-300',
			},
		},
		defaultVariants: {
			variant: 'list',
...
const contentVariants = cva('flex flex-col ', {
	variants: {
		variant: {
			list: 'items-start gap-2',
			card: 'flex-1 p-4 gap-5',
		},
	},
	defaultVariants: {
		variant: 'list',
...
const titleVariants = cva('', {
	variants: {
		variant: {
			list: 'lg:text-3xl sm:text-2xl text-xl font-bold tracking-tight leading-tight text-foreground',
			...
			{isCard ? (
				...
				>
					...
					<div className={contentVariants({ variant })}>
						{/* Header section */}
						<div className="flex h-full min-h-[80px] flex-col justify-between gap-2.5">
				...
				<div className={cn(resourceTeaserVariants({ variant }), className)}>
					...
					<div className={contentVariants({ variant })}>
						...
						{description && (
							<p className="text-muted-foreground max-w-[573px] text-sm leading-6 sm:text-base">
								{description}
							</p>
						)}
						{ctaText && (
