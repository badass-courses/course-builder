...
${url}`)}`}
			...
			<a
				href={`https://x.com/intent/tweet?text=${encodeURIComponent(`${displayedMessage}
...
${url}`)}`}
			...
			<a
				href={`https://linkedin.com/shareArticle?mini=true&url=${url}&title=${encodeURIComponent(displayedMessage)}&summary=${encodeURIComponent(post?.fields?.body?.slice(0, 200) ?? '')}`}
				target="_blank"
				rel="noopener noreferrer"
			>
				<svg
