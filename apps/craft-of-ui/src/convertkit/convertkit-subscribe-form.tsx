...
> = ({
	...
	return (
		<div
			data-waitlist-form
			className="relative mb-24 mt-20 scroll-mt-[calc(var(--header-height)*2)]"
			id={id}
		>
			<div
				...
				>
					...
					>
						...
						>
							<div
								className={cn(
									'pointer-events-none absolute inset-0 -translate-y-full [clip-path:inset(-200%_0_0px_0)]',
									'[container-type:inline-size]',
								)}
							>
								<svg
									className={cn(
										'absolute bottom-0 left-[55%] -z-20 w-12 translate-y-full',
										'motion-safe:[translate:0_calc(100%+(var(--show,0)*-50%))]',
										'delay-200 ease-out motion-safe:transition-transform',
									)}
									aria-hidden="true"
									viewBox="0 0 240 333"
									fill="none"
			...
			<output
				className={cn(
					'absolute left-0 right-0 top-full text-center text-sm outline-none',
					'data-[status=error]:text-red-400 [&_p]:text-balance',
					'opacity-100 blur-none motion-safe:data-[status=error]:transition-all',
					...
					'data-[status=success]:[&_p]:bg-green-200/10 data-[status=success]:[&_p]:text-green-800',
					'data-[status=success]:[&_p]:border data-[status=success]:[&_p]:border-green-800',
					'dark:data-[status=success]:[&_p]:bg-green-200/10 dark:data-[status=success]:[&_p]:text-green-200',
			...
			>
				{status === 'error' ? (
					<p>
						...
						<br />
						Your spot might not be saved — try again or refresh the page.
					</p>
				) : null}
				...
				{status === 'success' ? (
					<p>
						...
						<br />
						You’re on the waitlist. Please check your email to confirm your
						spot.
					</p>
				) : null}
			</output>
