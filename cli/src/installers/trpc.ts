...
export const trpcInstaller: Installer = ({ projectDir, packages, appRouter }) => {
  ...
  const trpcSrc = path.join(extrasDir, 'src/server/api', appRouter ? 'trpc-app' : 'trpc-pages', trpcFile)
