...
export async function orderDeepgramTranscript({videoResourceId, moduleSlug, mediaUrl}: {videoResourceId: string, moduleSlug?: string, mediaUrl: string}) {
  ...
  return {deepgram: await deepgramResponse.json(), postedUrl: `${deepgramUrl}?${deepgramParams.toString()}`}
}