...
export type WarningCode = 'debug-enabled' | 'env-url-basepath-redundant' | 'env-url-basepath-mismatch'
...
export const logger: LoggerInstance = {
  error(error) {
    ...
    if (error.cause && typeof error.cause === 'object' && 'err' in error.cause && error.cause.err instanceof Error) {
      ...
      if (data) console.error(`${red}[coursebuilder][details]${reset}:`, JSON.stringify(data, null, 2))
    } else if (error.stack) {
