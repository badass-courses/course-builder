...
export function mySqlDrizzleAdapter(
 ...
 `
		...
		// Event methods
		createEvent: async (data: AdapterEventData) => {
			const hash = guid()
			const newResourceId = `event-${hash}`
			...
			const newEvent = {
				...
				fields: {
					...
					slug:
						data.fields.slug ||
						`${data.fields.title.toLowerCase().replace(/\s+/g, '-')}-${hash}`,
					state: data.fields.state || 'draft',
					visibility: data.fields.visibility || 'public',
				},
			...
			const result = await client.transaction(async (tx) => {
				...
				const newEventSeries = {
					...
					fields: {
						...data.eventSeries,
						slug:
							data.eventSeries.slug ||
							`${data.eventSeries.title.toLowerCase().replace(/\s+/g, '-')}-${eventSeriesHash}`,
						state: data.eventSeries.state || 'draft',
						visibility: data.eventSeries.visibility || 'public',
					},
					createdById: data.createdById,
					createdByOrganizationMembershipId: null,
					createdAt: new Date(),
				...
				// Fetch the created event series with relationships
				...
				if (!eventSeriesResource) {
					throw new Error('Failed to create event series')
				}

				// Step 2: Create child events and associate them
				const createdChildEvents = []

				for (let i = 0; i < data.childEvents.length; i++) {
					const childEventInput = data.childEvents[i]
					...
					const childEventResourceId = `event-${childEventHash}`

					const newChildEvent = {
						id: childEventResourceId,
						organizationId: data.organizationId || null,
						type: childEventInput.type,
						fields: {
							...childEventInput.fields,
							slug: `${childEventInput.fields.title.toLowerCase().replace(/\s+/g, '-')}-${childEventHash}`,
							state: 'draft',
							visibility: 'public',
						},
						createdById: data.createdById,
					...
					// Fetch the created child event with relationships
					...
					if (!childEventResource) {
						throw new Error(`Failed to create child event ${i + 1}`)
					}

					// Associate child event with the event series
					...
					createdChildEvents.push(childEventResource)
				}

				return {
					eventSeries: EventSeriesSchema.parse(eventSeriesResource),
					childEvents: createdChildEvents.map((event) =>
