...
if [[ "$MODE" == "run" || "$MODE" == "watch" ]]; then
  ...
  NODE_OPTIONS='--import tsx' drizzle-kit generate --config=./test/mysql/drizzle.config.ts

  echo "Pushing MySQL schema..."
  NODE_OPTIONS='--import tsx' drizzle-kit push --config=./test/mysql/drizzle.config.ts
